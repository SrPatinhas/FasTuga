			...
			<form class="needs-validation px-3 px-sm-4 px-xl-5 pt-sm-1 pb-4 pb-sm-5" novalidate="" @submit.prevent>
				...
				<div class="row">
					<div class="col-sm-6 mb-4 mb-sm-0">
							...
							<div class="accordion-item border-bottom">
								<div class="accordion-header">
									<div class="d-table accordion-button" :class="orderCheckout.pay_type !== 'visa' && 'collapsed'" data-bs-toggle="collapse" data-bs-target="#credit-card" aria-expanded="false">
										<input class="form-check-input" type="radio" name="license" id="payment-card" value="visa"
											   :disabled="orderPaying > 200"
											   v-model="orderCheckout.pay_type" @change="orderCheckout.pay_reference = ''">
								...
								<div class="collapse" :class="orderCheckout.pay_type === 'visa' && 'show'" id="credit-card" data-bs-parent="#payment-methods">
									<div class="accordion-body py-2">
										<input class="form-control bg-image-none mb-3" type="text"
											   :disabled="orderPaying > 200"
											   v-model="orderCheckout.pay_reference" placeholder="4xxxxxxxxxxxxxxx">
									</div>
								</div>
							</div>
							<div class="accordion-item border-bottom">
								<div class="accordion-header">
									<div class="d-table accordion-button" :class="orderCheckout.pay_type !== 'paypal' && 'collapsed'" data-bs-toggle="collapse" data-bs-target="#paypal" aria-expanded="false">
										<input class="form-check-input" type="radio" name="license" id="payment-paypal" value="paypal"
											   :disabled="orderPaying > 50"
											   v-model="orderCheckout.pay_type" @change="orderCheckout.pay_reference = ''">
								...
								<div class="collapse" :class="orderCheckout.pay_type === 'paypal' && 'show'" id="paypal" data-bs-parent="#payment-methods">
									<div class="accordion-body pt-2">
										<input class="form-control" type="text" required="" id="fd-phone"
											   :disabled="orderPaying > 50"
											   v-model="orderCheckout.pay_reference" placeholder="email@email.com">
									</div>
								</div>
							</div>
							<div class="accordion-item border-bottom">
								<div class="accordion-header">
									<div class="d-table accordion-button" :class="orderCheckout.pay_type !== 'mbway' && 'collapsed'" data-bs-toggle="collapse" data-bs-target="#cash" aria-expanded="false">
										<input class="form-check-input" type="radio" name="license" id="payment-mbway" value="mbway"
											   :disabled="orderPaying > 10"
											   v-model="orderCheckout.pay_type" @change="orderCheckout.pay_reference = ''">
								...
								<div class="collapse" :class="orderCheckout.pay_type === 'mbway' && 'show'" id="cash" data-bs-parent="#payment-methods">
									<div class="accordion-body pt-2">
										<input class="form-control" type="text" required=""
											   :disabled="orderPaying > 10"
											   v-model="orderCheckout.pay_reference" placeholder="9xxxxxxxx">
									</div>
								</div>
							</div>
							<div class="accordion-item" v-if="userStore.isCustomer">
								<div class="accordion-header">
									<div class="accordion-button fw-medium collapsed" data-bs-toggle="collapse" data-bs-target="#points" aria-expanded="false">
										<i class="bi-gift me-2"></i>Redeem Reward Points ({{ userStore.availablePoints }})
									</div>
								</div>
						...
						<div class="align-items-center d-flex mb-3 small alert alert-secondary w-100" role="alert">
							<i class="mr-2 bi-info-circle"></i>
							<div>
								You can use the following methods: <br>
								<b>Visa</b>: until 200€<br>
								<b>Paypal</b>: until 50€<br>
								<b>MBway</b>: until 10€<br>
							</div>
						</div>
					</div>
					<div class="col-sm-6">
...
<script setup>
	...
	const orderPaying = computed(() => {
		return orderStore.totalOrderCost - (orderPoints.value / 2);
	});
	...
	* */
	...
	const submitOrder = async () => {
		if(canOrder.value) {
			orderCheckout.points = orderPoints.value;
	...
	const validatePayment = computed(() => {
		...
		if (paymentType === "visa" && orderPaying.value <= 200) { // for a Visa payment is the Visa Card ID with 16 digits;
			isPaymentValid = validations.validateVisaCard(paymentValue);
		} else if (paymentType === "paypal" && orderPaying.value <= 50) { // the reference for the PayPal is a valid email
			isPaymentValid = validations.isValidEmail(paymentValue);
		} else if (paymentType === "mbway" && orderPaying.value <= 10) { // the reference for the MbWay is the mobile phone number with 9 digits
			isPaymentValid = /^(9)[0-9]{8}$/g.test(paymentValue);//validations.isValidPhoneNumber(parseInt(paymentValue));
		}
		return isPaymentValid;
	});

	const isFormValid = computed(() => {
		let isValid = validateEmail.value && validatePhone.value && validateNif.value && validatePayment.value && orderStore.totalItems > 0;
		canOrder.value = isValid;
		return isValid;
	});
</script>